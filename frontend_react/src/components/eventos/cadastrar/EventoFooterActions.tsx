...
export function EventoFooterActions() {
  const navigate = useNavigate();
  const { submitForm, isSubmitting, formData } = useEventoForm();
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Erro desconhecido ao agendar encontro.');
    }
  };
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl">
            <p className="text-sm text-slate-700 font-medium mb-2">
              Você está prestes a agendar o encontro:
            </p>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-500 font-mono">
              <span>Data: {formData.data}</span>
              <span>•</span>
              <span>Horário: {formData.horarioInicio} às {formData.horarioFim}</span>
              <span>•</span>
              <span>Tipo: {formData.tipoEvento}</span>
            </div>
          </div>
